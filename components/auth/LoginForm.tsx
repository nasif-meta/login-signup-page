...
export default function LoginForm({ onSubmit, onToggleMode, onSocialLogin }: LoginFormProps) {
    ...
    >
      ...
      <div className="mb-5">
        <div className={`relative h-[52px] rounded-xl border transition-all duration-300 flex items-center overflow-hidden ${
          errors.email ? "border-red-500 shadow-[0_0_0_4px_rgba(239,68,68,0.08)]" : "border-white/[0.08] focus-within:border-indigo-500 focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.1)]"
        }`}>
          <svg className={`absolute left-4 w-5 h-5 transition-colors ${errors.email ? "text-red-500" : "text-slate-500"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <input
            ...
            placeholder=" "
            className="w-full h-full pl-12 pr-4 bg-transparent text-slate-100 text-sm outline-none"
          />
          <label className={`absolute left-12 transition-all duration-300 pointer-events-none bg-[#161628] px-1.5 rounded ${
            email ? "top-0 -translate-y-1/2 scale-[0.82] text-indigo-400 font-medium" : "top-1/2 -translate-y-1/2 text-slate-500"
          } ${errors.email ? "text-red-500!" : ""}`}>
            Email address
          </label>
          <div className="absolute bottom-0 left-1/2 w-0 h-[2.5px] bg-gradient-to-r from-indigo-500 to-purple-500 rounded-b-xl transition-all duration-500 group-focus-within:left-0 group-focus-within:w-full" />
        </div>
        {errors.email && (
      ...
      <div className="mb-5">
        <div className={`relative h-[52px] rounded-xl border transition-all duration-300 flex items-center overflow-hidden ${
          errors.password ? "border-red-500 shadow-[0_0_0_4px_rgba(239,68,68,0.08)]" : "border-white/[0.08] focus-within:border-indigo-500 focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.1)]"
        }`}>
          <svg className={`absolute left-4 w-5 h-5 transition-colors ${errors.password ? "text-red-500" : "text-slate-500"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          />
          <label className={`absolute left-12 transition-all duration-300 pointer-events-none bg-[#161628] px-1.5 rounded ${
            password ? "top-0 -translate-y-1/2 scale-[0.82] text-indigo-400 font-medium" : "top-1/2 -translate-y-1/2 text-slate-500"
          ...
          <button
            ...
            className="absolute right-3 p-1.5 text-slate-500 hover:text-slate-200 transition-colors rounded-lg hover:bg-white/5"
          >
            {showPassword ? (
      ...
      <div className="flex justify-between items-center mb-6">
        <label className="flex items-center gap-2.5 cursor-pointer">
          <input type="checkbox" className="w-[18px] h-[18px] rounded-md border-2 border-white/[0.08] bg-transparent checked:bg-gradient-to-br checked:from-indigo-500 checked:to-purple-500 checked:border-none appearance-none relative cursor-pointer transition-all checked:after:content-[''] checked:after:absolute checked:after:top-[2px] checked:after:left-[6px] checked:after:w-[4px] checked:after:h-[8px] checked:after:border-white checked:after:border-r-2 checked:after:border-b-2 checked:after:rotate-45" />
          <span className="text-[13px] text-slate-400">Remember me</span>
        </label>
        <a href="#" className="text-[13px] text-indigo-400 font-medium hover:underline">Forgot password?</a>
      </div>
